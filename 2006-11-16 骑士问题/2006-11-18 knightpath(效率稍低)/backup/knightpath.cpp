...
void knight_walk(int srci,int srcj,int desti,int destj)
{
	//初始化棋盘和其他参数
	initialize();
...
int min_step()
{
	int minstep=pathstep[0];
	for(int i=0;i<pathcount;i++)
	{
		if(pathstep[i]<minstep)
			minstep=pathstep[i];
	}
	return minstep;
}
		...
		for(srcj=1;srcj<=MAXLENGTH;srcj++)
		{
			for(desti=1;desti<=MAXLENGTH;desti++)
			{
				for(destj=1;destj<=MAXLENGTH;destj++)
				{
					...
					knight_walk(srci,srcj,desti,destj);
					minstep=min_step();
...
main()
{
	...
	while(stricmp(sinput,"q")!=0)
	{
		if(stricmp(sinput,"p")==0)
		{
			...
			//显示要搜索的路径个数及其最短路径个数
			printf("path number searced =%d, min path number = %d\n\n",pathcount,nMinStepCount);			
